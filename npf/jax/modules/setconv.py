...
class Discretization1d(nn.Module):
    ...
    def __call__(self,
        ...
        x_ctx_min = jnp.min(F.masked_min(x_ctx, mask_ctx, axis=-2))             # [1]
        x_ctx_max = jnp.max(F.masked_max(x_ctx, mask_ctx, axis=-2))             # [1]
        x_tar_min = jnp.min(F.masked_min(x_tar, mask_tar, axis=-2))             # [1]
        x_tar_max = jnp.max(F.masked_max(x_tar, mask_tar, axis=-2))             # [1]

        x_min = jnp.min(jnp.stack((x_ctx_min, x_tar_min)))
        x_max = jnp.max(jnp.stack((x_ctx_max, x_tar_max)))

        grid_min = self.minval - (self.margin + (1 / self.points_per_unit))
        grid_max = self.maxval + (self.margin + (1 / self.points_per_unit))

        data_min = x_min - (self.margin + (1 / self.points_per_unit))
        data_max = x_max + (self.margin + (1 / self.points_per_unit))
        ...
        mask_grid = (data_min <= x_grid) & (x_grid <= data_max)                 # [discrete]

        x_grid = np.expand_dims(x_grid, axis=(0, 2))                            # [1, discrete, 1]
...
class SetConv2dEncoder(SetConvBase):
    ...
    def __call__(self,
        ...
        weight = F.apply_mask(weight, mask, axis=-2)                            # [batch, source, target]
...
class SetConv2dDecoder(SetConvBase):
    ...
    def __call__(self,
        ...
        weight = self._get_weight(query, key)                                   # [batch, target, source]
        weight = F.apply_mask(weight, mask, axis=-1)                            # [batch, source, target]
        weight = jnp.expand_dims(weight, axis=-3)                               # [batch, 1, target, source]
